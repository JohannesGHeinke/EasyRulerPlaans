...
internal final class MeasureState_Handler: NSObject {
    ...
    internal final lazy var cancelButton = { () -> CancelButton in
        ...
        self.currentState.interact({ (controller) in
            ...
            button.setImage(UIImage.init(named: "cancel-button"), for: .normal)
        })
        return button
    }()
    ...
    internal final lazy var tapGestureImageView = { () -> TapGestureView in
        var gesture = TapGestureView.init(frame: CGRect.init(), handler: self)
        self.currentState.interact({ (controller) in
            gesture = TapGestureView.init(frame: CGRect(x: 0, y: controller.viewController.view.frame.size.height / 2, width: controller.viewController.view.frame.size.width, height: controller.viewController.view.frame.size.height / 2), handler: self)
            gesture.image = UIImage.init(named: "gesture-tap")
